'use strict';

const expressPlatby = require('../../testUtils.js').getSpyableExpressPlatby();

const expect = require('chai').expect;

describe('Checkout', () => {
  describe('Sessions Resource', () => {
    describe('create', () => {
      it('Sends the correct request', () => {
        const params = {
          cancel_url: 'https://expressplatby.cz/cancel',
          client_reference_id: '1234',
          line_items: [
            {
              amount: 123,
              currency: 'usd',
              description: 'item 1',
              images: ['https://expressplatby.cz/img1'],
              name: 'name',
              quantity: 2,
            },
          ],
          payment_intent_data: {
            receipt_email: 'test@expressplatby.cz',
          },
          payment_method_types: ['card'],
          success_url: 'https://expressplatby.cz/success',
        };
        expressPlatby.checkout.sessions.create(params);

        expect(expressPlatby.LAST_REQUEST).to.deep.equal({
          method: 'POST',
          url: '/v1/checkout/sessions',
          headers: {},
          data: params,
          settings: {},
        });
      });
    });

    describe('retrieve', () => {
      it('Sends the correct request', () => {
        expressPlatby.checkout.sessions.retrieve('cs_123');
        expect(expressPlatby.LAST_REQUEST).to.deep.equal({
          method: 'GET',
          url: '/v1/checkout/sessions/cs_123',
          data: {},
          headers: {},
          settings: {},
        });
      });
    });

    describe('listLineItems', () => {
      it('Sends the correct request', () => {
        expressPlatby.checkout.sessions.listLineItems('cs_123');
        expect(expressPlatby.LAST_REQUEST).to.deep.equal({
          method: 'GET',
          url: '/v1/checkout/sessions/cs_123/line_items',
          headers: {},
          data: {},
          settings: {},
        });
      });
    });
  });
});
