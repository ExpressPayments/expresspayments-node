'use strict';

const expressPlatby = require('../../testUtils.js').getSpyableExpressPlatby();
const expect = require('chai').expect;

describe('Issuing', () => {
  describe('Authorization Resource', () => {
    describe('retrieve', () => {
      it('Sends the correct request', () => {
        expressPlatby.issuing.authorizations.retrieve('iauth_123');
        expect(expressPlatby.LAST_REQUEST).to.deep.equal({
          method: 'GET',
          url: '/v1/issuing/authorizations/iauth_123',
          data: {},
          headers: {},
          settings: {},
        });
      });
    });

    describe('list', () => {
      it('Sends the correct request', () => {
        expressPlatby.issuing.authorizations.list();
        expect(expressPlatby.LAST_REQUEST).to.deep.equal({
          method: 'GET',
          url: '/v1/issuing/authorizations',
          data: {},
          headers: {},
          settings: {},
        });
      });
    });

    describe('update', () => {
      it('Sends the correct request', () => {
        expressPlatby.issuing.authorizations.update('iauth_123', {
          metadata: {
            thing1: true,
            thing2: 'yes',
          },
        });
        expect(expressPlatby.LAST_REQUEST).to.deep.equal({
          method: 'POST',
          url: '/v1/issuing/authorizations/iauth_123',
          headers: {},
          data: {
            metadata: {
              thing1: true,
              thing2: 'yes',
            },
          },
          settings: {},
        });
      });
    });

    describe('approve', () => {
      it('Sends the correct request', () => {
        expressPlatby.issuing.authorizations.approve('iauth_123');
        expect(expressPlatby.LAST_REQUEST).to.deep.equal({
          method: 'POST',
          url: '/v1/issuing/authorizations/iauth_123/approve',
          headers: {},
          data: {},
          settings: {},
        });
      });
    });

    describe('decline', () => {
      it('Sends the correct request', () => {
        expressPlatby.issuing.authorizations.decline('iauth_123');
        expect(expressPlatby.LAST_REQUEST).to.deep.equal({
          method: 'POST',
          url: '/v1/issuing/authorizations/iauth_123/decline',
          headers: {},
          data: {},
          settings: {},
        });
      });
    });
  });
});
