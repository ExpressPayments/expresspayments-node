...
// Resource integration tests which use expresspayments-mock.

const expressPayments = require('../testUtils.js').getExpressPaymentsMockClient();
...
describe('Charges Resource', () => {
    describe('search', () => {
        it('Retrieves first page', async () => {
            const result = await expressPayments.charges.search({
                query: 'currency:"USD"',
            });
        ...
        it('Retrieves all as array', async () => {
            const result = await expressPayments.charges
...
describe('Reader Resource', () => {
    describe('presentPaymentMethod', () => {
        it('Sends the correct request', async () => {
            const reader = await expressPayments.testHelpers.terminal.readers.presentPaymentMethod(
                'rdr_123'
            );
